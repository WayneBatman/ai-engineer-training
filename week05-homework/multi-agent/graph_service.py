...
from angent_state import AgentState



async def build_graph(mcp_session, test_mode: bool = False):
    mcp_tools = await load_mcp_tools(mcp_session)
    nodes = ArticleAgentNode(mcp_tools)
    nodes.test_mode = test_mode  # 设置测试模式
